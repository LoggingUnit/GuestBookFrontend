...
class FrontendContent {
    ...
    */
    refreshView(commentsArray) {
        ...
        for (let j = 0; j < mount.childNodes.length; j++) {
            var currentNode = mount.childNodes[j];
            if (!havePair(currentNode)) {
                currentNode.remove();
            }
            
            function havePair(currentNode) {
                for (var i=0; i < commentsArray.length; i++) {
                    var currentComment = commentsArray[i];
                    if (currentNode.id == currentComment.id) {
                        console.info("Node with ID: ", currentNode.id, " have pair");
                        return true;
                    }
                }
                console.info("Node with ID: ", currentNode.id, " have no pair and deleted");
                return false;
            }
